...
public class UIScenePolish : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        bool isGameplayScene = scene == "Desert" || scene == "Warzone";

        if (isGameplayScene && isHud)
        {
            TameHudBackground(canvas.transform);
            RemoveExistingProceduralBackground(canvas.transform);
        ...
        if (isHud)
        {
            ...
            // Không tạo nền procedural cho HUD gameplay.
    ...
    private static void RemoveExistingProceduralBackground(Transform canvas)
    {
        var bg = canvas.Find("ProceduralBG");
        if (bg != null)
        {
